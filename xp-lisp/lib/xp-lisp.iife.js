var lisp = (() => {
  ...
  function read_token(code2, exp2) {
    if (code2.length === 0) return void 0;
    const token = code2.shift();
    exp2.push(token);
    return token;
  }
  function read_list(code2, exp2, ch) {
    const result = [];
    let ast2;
    while ((ast2 = read_sexp(code2, exp2, false)) !== void 0) {
      if (ast2 === "]") {
        if (ch !== "[") code2.unshift("]");
        break;
      } else if (ast2 === ")") {
  ...
  function read_dict(code2, exp2) {
    const result = [["#", "dict"]];
    let ast1;
    let ast2;
    while ((ast1 = read_sexp(code2, exp2)) !== void 0) {
      if (ast1 === "]") continue;
      if (ast1 === "}") break;
      ast2 = read_sexp(code2, exp2);
      result.push(ast1);
      result.push(ast2);
    }
    return result;
  }
  function read_sexp(code2, exp2) {
    let token = read_token(code2, exp2);
    ...
    switch (ch) {
      case "(":
      case "[": {
        const lst = read_list(code2, exp2, ch);
        return lst;
      }
      case ")":
      case "]":
  ...
  function oml2ast(text2) {
    ...
    while (true) {
      const exp2 = [];
      const ast2 = read_sexp(code2, exp2);
  ...
  function compile_ast(ast2) {
    ...
    switch (common.to_id(ast2[0])) {
      ...
      case "def": {
        ...
        return "$scope." + common.to_id(ast2[1]) + "=" + compile_ast(ast2[2]);
      }
      case "define":
      case "defun":
      case "defvar": {
      ...
      case "let*": {
        ...
        return compile_ast(
          [common.id("_" + common.to_id(ast2[0])), new_ast1].concat(ast2.slice(2))
        );
      }
      case "_let":
      case "_let*": {
  ...
  function lisp1($scope, $system, $jsBeautify) {
    if (!$scope) $scope = {};
    if (!$jsBeautify) {
      $jsBeautify = function(code2) {
        return code2;
      };
    }
    $scope.evalJS = (code) => {
    ...
    $scope.compile_ast = (ast2, debug2) => {
      if (debug2) {
        console.error(" [AST] " + JSON.stringify(ast2));
      }
      const code2 = compile_ast(ast2);
      if (debug2) {
        console.error("<SCRIPT>\n" + $jsBeautify(code2) + "\n<\/SCRIPT>");
      }
      return code2;
    };
    $scope.compile = (text2, debug2) => {
    ...
    $scope.exec_d = (exp2) => $scope.exec(exp2, true);
    ...
    $scope.run = (exp2) => $scope.exec(exp2, true);
    ...
    $scope.runAll = (exp2) => {
      return $scope.execAll(exp2, true);
    };
    $scope.execFile = (path, debug2) => {
      const text2 = Deno.readTextFileSync(path);
      return $scope.execAll(text2, debug2);
    };
    $scope.runFile = (path) => {
      return $scope.execFile(path, true);
    };
    $scope.async_execURL = async (url, debug2) => {
  ...
  // standalone.js
  function version() {
    return "standalone:xp-lisp: version 2026.310.54532";
  }
  function versionNumber() {
    const split = version().split(" ");
    return split[2];
  }
  function lisp($scope2, $system2, $jsBeautify2) {
    return lisp1($scope2, $system2, $jsBeautify2);
  }
